#!/usr/bin/env python3
"""Batch v3: rebuild all PRF TDE sources + classify (no trimming, 6000 max_tokens, fixed parse)."""
...
print(f'Batch v3: {len(todo)} sources\n')

for i, (ztf_name, ra, dec) in enumerate(todo):
    sid = f'{ztf_name}_PRF'
    
    cmd = ['python3', 'ztf_adapter.py', ztf_name]
    if ra is not None: cmd += ['--ra', str(ra), '--dec', str(dec)]
    r = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
    bl = [l.strip() for l in r.stdout.split('\n') if 'Baseline:' in l]
    bl_info = bl[0] if bl else '?'
    
    npy_path = Path(f'../ZTF_TDE/data/TS/Flux/TDE/{ztf_name}_difference_photometry_flux.npy')
    arr = np.load(npy_path)
    idx[sid] = {'label': 'TDE', 'n_points': len(arr), 'source': 'PRF paper v3'}
    ...
    print(f'[{i+1:2d}/{len(todo)}] {sid:35s} {bl_info[:50]:50s} → {cls_info}')
