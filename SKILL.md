...
21. **External callers (TDEweb) should use `classify_pipeline()` not raw `sample_few_shot`/`build_prompt`/`call_api`/`parse_response`/`save_result`** (ADDED 2026-08-12): TDEweb had 3 code paths each manually calling all 5 functions + `load_index`. This caused triplicate bugs (missing baseline, wrong `max_tokens`). `classify_pipeline(source_id, model="qwen3.6-chat", cot=False)` encapsulates the full flow into a single call returning `{label, confidence, score, unsure_preference, primary_signal, indicators, flags, tokens, cot_reasoning}`. The pipeline internally handles `promt_load_index()`, `sample_few_shot()`, `build_prompt()`, `call_api()`, `parse_response()`, and `save_result()`. TDEweb now uses this in all 3 paths; adding a new caller means adding one import + one function call.
