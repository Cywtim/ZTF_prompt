...
def _make_system_prompt(cot=False, has_cutout=False):
    ...
    base = (
        ...
        "(rise→peak→decline), but distinguished by strong IR excess from the "
        "dust torus (WISE W1−W2 typically > 0.8). Always nuclear (gaia_sep ≈ 0). "
        ...
        "## Decision Logic\n"
        "0. CHECK WISE FIRST (if available): if W1−W2 ≥ 0.8 AND the light curve\n"
        "   is TDE-like (color/shape), classify as AGN, not TDE. This is the single\n"
        "   strongest TDE/AGN discriminator — the dust torus IR excess is definitive.\n"
        "   If WISE is unavailable, proceed to step 1.\n"
        ...
        "5. Gaia separation (if available): small sep (< 2 arcsec) favors TDE/AGN\n"
        "   (nuclear events); large sep favors SN. Use as weak tiebreaker.\n"
        "6. WISE W1−W2 (if available): < 0.5 → TDE, ≥ 0.8 → AGN. Critical for\n"
        "   TDE/AGN disambiguation when light curve shape is ambiguous.\n"
        "7. WHEN TO USE 'Unsure': if 2+ physical indicators point in opposite\n"
        ...
        "   clear signal exists.\n" +
        (
        "8. Host galaxy (if cutout available): use as weak tiebreaker ONLY.\n"
    ...
    if cot:
            cot_section = (
                ...
                "Step 5 — Gaia Separation: check Section 1 for 'Gaia Sep'.\n"
                "If available: < 2 arcsec → nuclear (favors TDE/AGN);\n"
                "≥ 2 arcsec → disk (favors SN). If absent, skip this step.\n"
                "Step 6 — WISE IR Color: check Section 1 for 'WISE W1/W2/W1-W2'.\n"
                "If available: W1−W2 < 0.5 → TDE; 0.5–0.8 → ambiguous;\n"
                "≥ 0.8 → AGN (key TDE/AGN degeneracy breaker). If absent, skip.\n" +
                (
                "Step 7 — Host Galaxy (from cutout): describe the host. "
                "Red/smooth/elliptical → favors TDE. Blue/spiral → favors SN. "
                "No obvious host → skip this step.\n"
                if has_cutout else ""
                ) +
                "Step " + ("8" if has_cutout else "7") + " — Synthesis: weigh all indicators. If they conflict, "
